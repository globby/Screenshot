...
parser.add_argument("-d","--delay",type=int,help="Wait <delay> seconds before taking shot")
parser.add_argument("-f","--filename",help="Specify a filename (default is a timestamp)")
parser.add_argument("-c","--clean",action="store_true",help="Delete screenshot after upload (default false)")
parser.add_argument("-q","--quiet",action="store_true",help="No output except the link")
parser.add_argument("-s","--select",action="store_true",help="Select a sub area of the screen")
parser.add_argument("-v","--verbose",action="store_true",help="Display additional progress information")
parser.add_argument("-l","--clipboard",action="store_true",help="Copy imgur link to clipboard (requires xclip)")
...
def takeScreenshot():
    ...
    if not re.search(r"^scrot version \d+\.\d+",resp):
        ...
        if args.select:
            command += "-s "
        FILENAME = args.filename if args.filename else time.strftime("%Y.%m.%d.%H.%M.%S.png",time.localtime())
        command += FILENAME
	...
	FILENAME = "%s/%s" % (os.getcwd(),FILENAME)
        if resp:
            ...
            if os.path.exists(FILENAME):
                ...
                printe("Error: The screenshot file seems to have dissapeared... what did you do???")        
def uploadScreenshot(screenshot):
    ...
    if int(resp.info()["X-RateLimit-ClientRemaining"])< 1:                                          #Haven't actually gotten here so not 100% sure it will work. If someone gets
        printe("You have run out of credits!")                                                      #to this point and it fails please leave a bug report on github
